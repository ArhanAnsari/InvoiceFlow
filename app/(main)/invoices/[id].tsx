...
export default function InvoiceDetailScreen() {
  ...
  if (!invoice) {
    return (
      <LinearGradient colors={isDark ? ["#0D0F1E", "#131629"] : ["#EEF2FF", "#F5F7FA"]} style={styles.root}>
  ...
  return (
    <LinearGradient
      colors={isDark ? ["#0D0F1E", "#131629", "#0D0F1E"] : ["#EEF2FF", "#F5F7FA", "#F0F4FF"]}
      style={styles.root}
    >
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
                ...
                {items.map((item: any, idx: number) => (
                  <View key={idx}>
                    <View style={styles.lineRow}>
                      <Text style={styles.lineProduct} numberOfLines={1}>{item.productName}</Text>
                      <Text style={styles.lineQty}>{item.quantity} × ₹{parseFloat(item.price).toLocaleString()}</Text>
                      <Text style={styles.lineTotal}>₹{parseFloat(item.totalPrice).toLocaleString()}</Text>
                    </View>
          ...
          <GlassCard dark={isDark} style={styles.summary}>
            ...
            <View style={styles.summaryRow}>
              ...
              <Text style={styles.summaryValue}>₹{totalTax.toLocaleString()}</Text>
            </View>
            {discountAmount > 0 && (
              <View style={styles.summaryRow}>
                <Text style={styles.summaryLabel}>Discount</Text>
                <Text style={[styles.summaryValue, { color: T.success }]}>-₹{discountAmount.toLocaleString()}</Text>
              </View>
            )}
            ...
            {paidAmount > 0 && (
              <View style={styles.summaryRow}>
                ...
                <Text style={[styles.summaryValue, { color: T.success }]}>₹{paidAmount.toLocaleString()}</Text>
              </View>
            )}
            {balanceDue > 0 && (
              <View style={styles.summaryRow}>
                <Text style={[styles.totalLabel, { color: T.danger }]}>Balance Due</Text>
                <Text style={[styles.totalValue, { color: T.danger }]}>₹{balanceDue.toLocaleString()}</Text>
              </View>
            )}
          </GlassCard>
...
function createStyles(T: typeof Colors.dark) { return StyleSheet.create({
  root: { flex: 1 },
  centered: { flex: 1, alignItems: "center", justifyContent: "center", gap: 12 },
  notFoundText: { ...Typography.h3, color: T.textMuted },
  backPill: { paddingHorizontal: 20, paddingVertical: 10, borderRadius: Radius.full, borderWidth: 1.5, borderColor: T.primary },
  backPillText: { color: T.primary, fontWeight: "700" },
  header: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: Spacing.xl, paddingTop: Spacing.lg, marginBottom: Spacing.md },
  ...
  sectionLabel: { fontSize: 12, fontWeight: "700", color: T.textMuted, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: Spacing.md, marginTop: Spacing.md },
  lineRow: { flexDirection: "row", alignItems: "center", paddingHorizontal: 14, paddingVertical: 12, gap: 8 },
  lineProduct: { flex: 1, fontSize: 14, color: T.text, fontWeight: "500" },
  lineQty: { fontSize: 12, color: T.textMuted },
  lineTotal: { fontSize: 14, color: T.text, fontWeight: "700", minWidth: 70, textAlign: "right" },
